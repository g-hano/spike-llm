...
class SpikingMoEFFN(nn.Module):
    def __init__(self, d_model, hidden_dim, num_experts=8, num_active=2, beta=0.95, num_steps=5, dtype=torch.bfloat16, device="cuda"):
        super().__init__()
        ...
        self.gate_spike = snn.Leaky(beta=beta, learn_beta=True)
        self.gate_scale = nn.Parameter(torch.ones(1, dtype=dtype, device=device))

        self.experts = nn.ModuleList([
            SpikingSwiGLU(d_model, hidden_dim, beta=beta, num_steps=num_steps, dtype=dtype, device=device)
    ...
    def forward(self, x):
        ...
        mem = self.gate_spike.init_leaky()
        spike_acc = torch.zeros_like(gate_logits)

        for _ in range(self.num_steps):
            spk, mem = self.gate_spike(gate_logits, mem)
            spike_acc += spk
        
        spike_rates = (spike_acc / self.num_steps) * self.gate_scale

        topk_logits, topk_indices = torch.topk(spike_rates, self.num_active, dim=-1)
...
class SpikingMoETransformerBlock(nn.Module):
    def __init__(self, d_model, n_heads, n_kv_heads, intermediate_size=None,
                 ...
                 rope_theta_global=1e6, window_size=128, beta=0.95, num_steps=5,
        ...
        if use_moe:
            self.ffn = SpikingMoEFFN(d_model, hidden_dim, num_experts, num_active, beta, num_steps=num_steps, dtype=dtype, device=device)
        else:
            self.ffn = SpikingSwiGLU(d_model, hidden_dim, beta, num_steps=num_steps, dtype=dtype, device=device)
    ...
    def __init__(self, vocab_size, d_model, n_heads, n_kv_heads, num_layers,
        ...
        self.layers = nn.ModuleList([
            SpikingMoETransformerBlock(
                ...
                window_size=window_size, beta=beta, num_steps=num_steps,
